...
- [ ] The local closeout contains a `symphony-outcome` block and reports `artifact_execution_only`.
...
```bash
runpod-bridge validate-manifest examples/proxy-matrix/launch_manifest.json
runpod-bridge plan examples/proxy-matrix/launch_manifest.json
runpod-bridge prepare examples/proxy-matrix/launch_manifest.json --out-dir .runtime/proxy-matrix-packet
bash -n .runtime/proxy-matrix-packet/startup.sh
runpod-bridge run-local examples/proxy-matrix/launch_manifest.json --repo-dir .runtime/proxy-matrix-local-repo --runtime-dir .runtime/proxy-matrix-local-run
runpod-bridge validate-linear-issue examples/proxy-matrix/linear_issue.md
```
