...
- The SSH connection command gap is resolved when `runpodctl` is installed: use `runpod-bridge pod-ssh-info <pod-id>` as a wrapper around `runpodctl ssh info`.
- RunPod GraphQL exposes read-only pod runtime metrics. Use `runtime-metrics` to catch crash loops that REST `get-pod` cannot distinguish from slow work. The bridge sends a browser-style User-Agent by default; override `RUNPOD_GRAPHQL_USER_AGENT` only if the operator environment rewrites or blocks it.
...
- The committed bridge CLI now covers local `doctor`, `validate-manifest`, `contract-self-check`, `preflight`, `profiles`, `provider-capabilities`, `linear-issue`, `linear-comment`, `render-startup`, `render-runpodctl-create`, `plan`, `prepare`, `write-handoff`, `validate-handoff`, `issue-intake`, `run-local`, `monitor`, `supervise`, `dashboard`, and `closeout`.
- The committed bridge CLI also includes guarded RunPod REST/GraphQL operations: `run-handoff`, `run-remote`, `orchestrator-scan`, `orchestrator-once`, `create-pod`, `list-pods`, `get-pod`, `runtime-metrics`, `billing-pods`, `billing-endpoints`, `billing-network-volumes`, `cost-report`, `list-network-volumes`, `get-network-volume`, `list-templates`, `get-template`, `recover-run`, and `cleanup-pod`.
...
- Workspace archive egress and generic object-store archive upload are implemented. SCP and network-volume-specific copy helpers remain adapter work.
- HTTP proxy and direct TCP packet fetching are implemented for sanitized smoke inspection, but they are not reliable enough to be the only production artifact channel.
...
- A repo-local `runpod-bridge` CLI or equivalent scripts for manifest validation, startup rendering, dry-run planning, pod monitoring, artifact hashing, and closeout.
- Run `runpod-bridge doctor` before handing a RunPod issue to Symphony. Warnings are acceptable for local dry-runs, but failures mean the worker will probably not discover the bridge.
- Use `runpod-bridge prepare` to emit `launch_manifest.json`, `startup.sh`, `local_preflight.json`, and `provider_handoff.json` before any paid launch.
- Use `runpod-bridge contract-self-check`, `runpod-bridge source-check --execute`, `runpod-bridge preflight`, and `runpod-bridge egress-plan` before assigning large or huge workloads.
- Use `runpod-bridge validate-handoff` on `provider_handoff.json` before passing it to an orchestrator.
- Use `runpod-bridge issue-intake` when the work starts from a Linear issue body and manifest pair.
- Use `runpod-bridge run-remote` for orchestrator-side paid smokes so create, packet verification, and cleanup land in one audit record.
- Use `runpod-bridge run-handoff` when the orchestrator is consuming a worker-produced packet.
- Use `runpod-bridge orchestrator-scan` or `runpod-bridge orchestrator-once` for a local handoff queue.
- Use `runpod-bridge create-pod` without `--execute` when you need to review the exact REST request before any paid launch.
- For Symphony workers, run a cheap network preflight before assigning remote mutation: `runpod-bridge list-pods --name-prefix definitely-no-match --json`. If it fails with DNS or connection errors, do not retry inside the worker; have the worker close out with the prepared packet and let an orchestrator-side lane run `run-handoff` and Linear finalization.
- Use `runpod-bridge validate-linear-issue` on repo-provided issue bodies before handing them to Symphony.
- Use `runpod-bridge productivity-plan` before long remote launches to confirm the manifest has a live progress channel. Use `runpod-bridge supervise` for long-running local packets and `runpod-bridge dashboard` for multi-run operator views.
- Use `runpod-bridge runtime-metrics <pod-id> --expected-elapsed-minutes <minutes> --json` shortly after allocation and again a few minutes later. If `uptimeInSeconds` resets or stays near zero after long elapsed time, treat it as a crash loop and clean up unless an operator can inspect logs immediately.
- Use `runpod-bridge cost-report --fetch-billing` after remote cleanup when the RunPod billing API is reachable.
- Use `runpod-bridge billing-endpoints` and `runpod-bridge billing-network-volumes` for non-pod cost closeout when Serverless endpoints or retained volumes are involved.
- Use `runpod-bridge billing-pods --backend runpodctl`, `billing-endpoints --backend runpodctl`, or `billing-network-volumes --backend runpodctl` when the operator host has `runpodctl` configured.
- Use `runpod-bridge render-runpodctl-create <manifest>` to inspect the equivalent `runpodctl pod create` command and confirm both `--docker-args` startup execution and `--terminate-after` are present when platform-side deletion backstop is required.
- Use `runpod-bridge pod-ssh-info <pod-id>` to fetch SSH details through `runpodctl ssh info`; do not paste private key material or one-time transfer codes into Linear.
- Use `runpod-bridge recover-run` on failed or interrupted run records before manually inspecting the console.
- Use `runpod-bridge verify-proxy-packet` or `runpod-bridge verify-tcp-packet` only for short-lived, sanitized smokes.
...
- `runpod-execution/artifact_hashes.json` for SHA-256 hashes
...
- Git bootstrap requires `git` in the image before `startup.commands` run. Exact images should be canaried and declared with `runpod.image_capabilities: ["git"]`; otherwise use inline/snapshot/object-store bootstrap.
...
- Local dry-run should use `runpod-bridge run-local` first; it executes the rendered startup contract with `RUNPOD_REPO_DIR` pointed at a local repo directory and produces the same execution packet shape.
...
- `runpod_network_volume_s3`: write artifacts to an attached network volume, delete the pod, then pull artifacts through RunPod's S3-compatible API with runtime-injected S3 API credentials.
...
For `object_store_upload`, the startup script can upload the declared archive and hash file when `RUNPOD_OBJECT_STORE_URI`, an AWS-compatible CLI, and runtime credentials are present. If `requires_object_store_upload` is true, missing upload configuration fails the workload.
