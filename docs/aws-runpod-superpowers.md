...
Checked against official RunPod and AWS docs on 2026-05-02.
...
Use `runpod-bridge aws-orchestrator-plan <manifest>` before adding AWS-backed orchestration. It renders command templates, helper JSON templates, and required environment refs for every companion surface below without executing AWS or RunPod mutations. When helper files contain `$ENV` placeholders, the rendered plan includes a `python3` expansion step so the final AWS CLI commands receive concrete JSON only on the trusted orchestrator host.
...
- `examples/runpod-network-volume-s3/launch_manifest.json` demonstrates post-cleanup artifact pull through RunPod's S3-compatible network-volume API.
...
1. `aws_s3_presigned_upload` artifact egress
   ...
   - The pod uses `curl --upload-file`; closeout validates S3 object presence and hashes from the orchestrator side.
...
3. RunPod network-volume S3 plus AWS-compatible tooling
   - Use when artifacts should survive pod deletion and be pulled after cleanup.
