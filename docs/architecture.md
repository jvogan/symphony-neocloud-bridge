...
SSH connection command discovery is available when `runpodctl` is installed through `runpod-bridge pod-ssh-info <pod-id>`.
...
Report success only when declared artifact checks pass and cleanup status is recorded.
