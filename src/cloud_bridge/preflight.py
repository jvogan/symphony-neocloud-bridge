...
from .providers.runpod.rest import build_create_pod_request, build_remote_launch_preview
...
def analyze_preflight(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for blocker in launch_preview.get("source_gate", {}).get("blockers", []):
        errors.append({"severity": "error", "path": "source_gate", "message": blocker})
    for blocker in launch_preview.get("artifact_retrieval_gate", {}).get("blockers", []):
        errors.append({"severity": "error", "path": "artifact_retrieval", "message": blocker})
