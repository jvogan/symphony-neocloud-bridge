...
def supervise_execution(manifest: dict[str, Any], base_dir: str | Path = ".") -> dict[str, Any]:
    ...
    closeout = build_closeout(manifest, base) if monitor["state"] in ("terminal_reported", "failed") else {}
...
def next_action(monitor: dict[str, Any], closeout: dict[str, Any], checkpoint: dict[str, Any]) -> str:
    ...
    productivity_state = str((monitor.get("productivity") or {}).get("state") or "")
    if state == "terminal_reported" and closeout.get("status") == "succeeded":
    ...
    if state == "running":
        if productivity_state == "workload_progressing":
            return "continue_monitoring"
        if productivity_state == "harness_progress_unproven":
            return "poll_provider_runtime_and_live_progress_again"
        if productivity_state == "harness_alive_unproven":
            return "require_advancing_workload_signal_before_claiming_progress"
        return "poll_provider_runtime_and_workload_health"
