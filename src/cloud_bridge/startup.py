...
def render_startup_script(manifest: dict[str, Any]) -> str:
    ...
    if not object_store_uri.startswith("s3://"):
        object_store_uri = ""
    object_store_uri_env = env_name_from_ref(str(artifact_egress.get("destination_uri_ref") or ""), "RUNPOD_OBJECT_STORE_URI")
    ...
    source_archive_url_default = bash_parameter_default(str(snapshot.get("archive_url") or ""))
    source_archive_url_env_default = bash_parameter_default(archive_url_env)
    source_archive_path_default = bash_parameter_default(str(snapshot.get("archive_pod_path") or "/tmp/runpod-bridge-source.tar.gz"))
    source_archive_sha = str(snapshot.get("archive_sha256") or "")
    if not source_archive_sha and str(repo.get("commit_or_snapshot") or "").startswith("sha256:"):
        source_archive_sha = str(repo.get("commit_or_snapshot")).split(":", 1)[1]
    source_archive_sha_default = bash_parameter_default(source_archive_sha)
    archive_default = bash_parameter_default(archive_path)
    object_store_default = bash_parameter_default(object_store_uri)
    object_store_uri_env_default = bash_parameter_default(object_store_uri_env)
    ...
    terminal_hold = startup.get("terminal_hold", {}) if isinstance(startup.get("terminal_hold"), dict) else {}
    terminal_hold_mode = str(terminal_hold.get("mode") or "none")
    terminal_hold_seconds = int(terminal_hold.get("seconds") or 0)
    terminal_hold_on_success = "1" if terminal_hold.get("on_success", True) is not False else "0"
    terminal_hold_on_failure = "1" if terminal_hold.get("on_failure", True) is not False else "0"
...
RUNPOD_SOURCE_ARCHIVE_URL="${{RUNPOD_SOURCE_ARCHIVE_URL:-{source_archive_url_default}}}"
RUNPOD_SOURCE_ARCHIVE_URL_ENV="${{RUNPOD_SOURCE_ARCHIVE_URL_ENV:-{source_archive_url_env_default}}}"
RUNPOD_SOURCE_ARCHIVE_SHA256="${{RUNPOD_SOURCE_ARCHIVE_SHA256:-{source_archive_sha_default}}}"
RUNPOD_SOURCE_ARCHIVE_PATH="${{RUNPOD_SOURCE_ARCHIVE_PATH:-{source_archive_path_default}}}"
...
RUNPOD_OBJECT_STORE_URI_ENV="${{RUNPOD_OBJECT_STORE_URI_ENV:-{object_store_uri_env_default}}}"
...
RUNPOD_TERMINAL_HOLD_MODE="${{RUNPOD_TERMINAL_HOLD_MODE:-{terminal_hold_mode}}}"
RUNPOD_TERMINAL_HOLD_SECONDS="${{RUNPOD_TERMINAL_HOLD_SECONDS:-{terminal_hold_seconds}}}"
RUNPOD_TERMINAL_HOLD_ON_SUCCESS="${{RUNPOD_TERMINAL_HOLD_ON_SUCCESS:-{terminal_hold_on_success}}}"
RUNPOD_TERMINAL_HOLD_ON_FAILURE="${{RUNPOD_TERMINAL_HOLD_ON_FAILURE:-{terminal_hold_on_failure}}}"
RUNPOD_TERMINAL_HOLD_RAN=0
...
download_source_archive() {{
  if command -v curl >/dev/null 2>&1; then
    run_bootstrap_cmd curl --fail --silent --show-error --location --connect-timeout 30 --max-time "$RUNPOD_BOOTSTRAP_TIMEOUT_SECONDS" "$RUNPOD_SOURCE_ARCHIVE_URL" --output "$RUNPOD_SOURCE_ARCHIVE_PATH"
    return "$?"
  fi
  python_bin="$(command -v python3 || command -v python || true)"
  if [ -n "$python_bin" ]; then
    run_bootstrap_cmd "$python_bin" - "$RUNPOD_SOURCE_ARCHIVE_URL" "$RUNPOD_SOURCE_ARCHIVE_PATH" <<'RUNPOD_BRIDGE_DOWNLOAD_SOURCE'
import pathlib
import shutil
import sys
...
print(f"downloaded source archive to {{target}} ({{target.stat().st_size}} bytes)")
RUNPOD_BRIDGE_DOWNLOAD_SOURCE
    return "$?"
  fi
  echo "curl or python is required for prepared snapshot bootstrap" >&2
  return 127
}}

resolve_source_archive_url() {{
  if [ -z "$RUNPOD_SOURCE_ARCHIVE_URL" ] && [ -n "$RUNPOD_SOURCE_ARCHIVE_URL_ENV" ]; then
    RUNPOD_SOURCE_ARCHIVE_URL="${{!RUNPOD_SOURCE_ARCHIVE_URL_ENV:-}}"
  fi
}}

verify_source_archive() {{
  if [ -z "$RUNPOD_SOURCE_ARCHIVE_SHA256" ]; then
    return 0
  fi
  if command -v sha256sum >/dev/null 2>&1; then
    actual="$(sha256sum "$RUNPOD_SOURCE_ARCHIVE_PATH" | awk '{{print $1}}')"
  else
    actual="$(shasum -a 256 "$RUNPOD_SOURCE_ARCHIVE_PATH" | awk '{{print $1}}')"
  fi
  if [ "$actual" != "$RUNPOD_SOURCE_ARCHIVE_SHA256" ]; then
    bootstrap_fail 98 "source archive sha256 mismatch"
  fi
}}
...
bootstrap_repo() {{
  ...
  if [ "$RUNPOD_REPO_SOURCE" = "git_remote" ] || [ "$RUNPOD_REPO_SOURCE" = "git_remote_or_snapshot" ]; then
    if [ -z "$RUNPOD_REPO_URL" ] || [ -z "$RUNPOD_REPO_REF" ]; then
      bootstrap_fail 91 "RUNPOD_REPO_URL and RUNPOD_REPO_REF are required for remote git bootstrap"
    fi
  ...
  if [ "$RUNPOD_REPO_SOURCE" = "prepared_snapshot" ] || [ "$RUNPOD_REPO_SOURCE" = "object_store_archive" ]; then
    resolve_source_archive_url
    if [ -z "$RUNPOD_SOURCE_ARCHIVE_URL" ] && [ ! -f "$RUNPOD_SOURCE_ARCHIVE_PATH" ]; then
      bootstrap_fail 95 "prepared snapshot bootstrap requires RUNPOD_SOURCE_ARCHIVE_URL or an existing RUNPOD_SOURCE_ARCHIVE_PATH"
    fi
    command -v tar >/dev/null 2>&1 || bootstrap_fail 97 "tar is required for prepared snapshot bootstrap"
    bootstrap_event "begin"
    if [ -n "$RUNPOD_SOURCE_ARCHIVE_URL" ]; then
      bootstrap_log "downloading prepared source archive"
      mkdir -p "$(dirname "$RUNPOD_SOURCE_ARCHIVE_PATH")"
      download_source_archive >> "$RUNPOD_LOG_FILE" 2>&1 || bootstrap_fail 93 "source archive download failed or timed out"
    else
      bootstrap_log "using prepared source archive at $RUNPOD_SOURCE_ARCHIVE_PATH"
    fi
    verify_source_archive
    rm -rf "$RUNPOD_REPO_DIR"
    mkdir -p "$RUNPOD_REPO_DIR"
    tar -xzf "$RUNPOD_SOURCE_ARCHIVE_PATH" -C "$RUNPOD_REPO_DIR" >> "$RUNPOD_LOG_FILE" 2>&1 || bootstrap_fail 94 "source archive unpack failed"
    cd "$RUNPOD_REPO_DIR"
    bootstrap_event "succeeded"
    return
  fi
  bootstrap_fail 90 "RUNPOD_ENABLE_REPO_BOOTSTRAP=1 requires repo.source=git_remote or prepared_snapshot"
}}
...
event() {{
  phase="$1"
  status="$2"
  printf '{{"ts":"%s","phase":"%s","status":"%s","source":"cloud_bridge"}}\\n' "$(timestamp)" "$phase" "$status" >> "$RUNPOD_HEARTBEAT_FILE"
}}
...
write_status() {{
  ...
  exit_code="$2"
  phase="${{3:-$status}}"
  printf '{{"ts":"%s","status":"%s","phase":"%s","exit_code":%s,"log_file":"%s","hash_file":"%s"}}\\n' "$(timestamp)" "$status" "$phase" "$exit_code" "$RUNPOD_LOG_FILE" "$RUNPOD_HASH_FILE" > "$RUNPOD_STATUS_FILE"
}}
...
presigned_upload_file() {{
  ...
  upload_url="$2"
  if command -v curl >/dev/null 2>&1; then
    curl --fail --silent --show-error --upload-file "$upload_file_path" "$upload_url" >> "$RUNPOD_LOG_FILE" 2>&1
    return "$?"
  fi
  python_bin="$(command -v python3 || command -v python || true)"
  ...
  "$python_bin" - "$upload_file_path" "$upload_url" >> "$RUNPOD_LOG_FILE" 2>&1 <<'PY'
import pathlib
import sys
import urllib.request
...
with urllib.request.urlopen(request, timeout=300) as response:
    response.read()
print("uploaded %s (%d bytes) through presigned URL" % (file_path, len(data)))
PY
}}

resolve_object_store_env() {{
  if [ -z "$RUNPOD_OBJECT_STORE_URI" ] && [ -n "$RUNPOD_OBJECT_STORE_URI_ENV" ]; then
    RUNPOD_OBJECT_STORE_URI="${{!RUNPOD_OBJECT_STORE_URI_ENV:-}}"
  fi
}}
...
durable_egress() {{
  ...
  if [ "$RUNPOD_EGRESS_MODE" = "runpod_network_volume_s3" ]; then
    write_egress_status "retained" "artifacts written to attached network volume for S3-compatible pull"
    return 0
  fi
  if [ "$RUNPOD_EGRESS_MODE" = "aws_s3_presigned_upload" ]; then
    resolve_presigned_upload_env
    ...
    if ! command -v curl >/dev/null 2>&1 && ! command -v python3 >/dev/null 2>&1 && ! command -v python >/dev/null 2>&1; then
      write_egress_status "blocked" "curl or python is required for aws_s3_presigned_upload"
      [ "$RUNPOD_PRESIGNED_UPLOAD_REQUIRED" = "1" ] && return 77
      return 0
    fi
    if ! presigned_upload_file "$RUNPOD_ARCHIVE_PATH" "$RUNPOD_PRESIGNED_ARCHIVE_PUT_URL"; then
      write_egress_status "failed" "archive presigned upload failed"
      return 79
    fi
    if [ "$RUNPOD_PRESIGNED_HASH_UPLOAD_DECLARED" = "1" ]; then
      ...
      if [ -f "$RUNPOD_HASH_FILE" ]; then
        if ! presigned_upload_file "$RUNPOD_HASH_FILE" "$RUNPOD_PRESIGNED_HASH_PUT_URL"; then
          write_egress_status "failed" "hash presigned upload failed"
          return 80
        fi
      fi
    fi
  ...
  if [ -n "$RUNPOD_ARCHIVE_PATH" ] && [ -f "$RUNPOD_ARCHIVE_PATH" ]; then
    if ! aws s3 cp "$RUNPOD_ARCHIVE_PATH" "${{RUNPOD_OBJECT_STORE_URI%/}}/" >> "$RUNPOD_LOG_FILE" 2>&1; then
      write_egress_status "failed" "archive object-store upload failed"
      return 81
    fi
  fi
  if [ -f "$RUNPOD_HASH_FILE" ]; then
    if ! aws s3 cp "$RUNPOD_HASH_FILE" "${{RUNPOD_OBJECT_STORE_URI%/}}/" >> "$RUNPOD_LOG_FILE" 2>&1; then
      write_egress_status "failed" "hash object-store upload failed"
      return 82
    fi
  fi
...
progress_server() {{
  ...
  export RUNPOD_PROGRESS_SERVER_PORT RUNPOD_STATUS_FILE RUNPOD_HEARTBEAT_FILE RUNPOD_LOG_FILE RUNPOD_HASH_FILE
  export RUNPOD_PROGRESS_TOKEN_ENV RUNPOD_PROGRESS_INCLUDE_LOG_TAIL RUNPOD_PROGRESS_LOG_TAIL_BYTES
...
terminal_hold() {{
  if [ "$RUNPOD_TERMINAL_HOLD_MODE" = "none" ] || [ -z "$RUNPOD_TERMINAL_HOLD_MODE" ]; then
    return 0
  fi
  if [ "$RUNPOD_EXIT_CODE" -eq 0 ] && [ "$RUNPOD_TERMINAL_HOLD_ON_SUCCESS" != "1" ]; then
    return 0
  fi
  if [ "$RUNPOD_EXIT_CODE" -ne 0 ] && [ "$RUNPOD_TERMINAL_HOLD_ON_FAILURE" != "1" ]; then
    return 0
  fi
  RUNPOD_TERMINAL_HOLD_RAN=1
  event "terminal_hold" "begin"
  RUNPOD_HTTP_PID=""
  if [ "$RUNPOD_HTTP_ARTIFACT_SERVER_PORT" -gt 0 ]; then
    python3 -m http.server "$RUNPOD_HTTP_ARTIFACT_SERVER_PORT" --bind 0.0.0.0 >> "$RUNPOD_LOG_FILE" 2>&1 &
    RUNPOD_HTTP_PID="$!"
  ...
  if [ "$RUNPOD_TERMINAL_HOLD_MODE" = "sleep_infinity" ]; then
    event "terminal_hold" "sleep_infinity"
    if ! sleep infinity 2>/dev/null; then
      while true; do
        sleep 3600
      done
    fi
  elif [ "$RUNPOD_TERMINAL_HOLD_SECONDS" -gt 0 ]; then
    event "terminal_hold" "sleep_seconds"
    sleep "$RUNPOD_TERMINAL_HOLD_SECONDS"
  fi
  if [ -n "$RUNPOD_HTTP_PID" ]; then
    kill "$RUNPOD_HTTP_PID" >/dev/null 2>&1 || true
    wait "$RUNPOD_HTTP_PID" >/dev/null 2>&1 || true
  fi
  event "terminal_hold" "ended"
}}
...
event "startup" "begin"
write_status "running" 0 "startup"
heartbeat_loop &
...
if [ "$RUNPOD_EXIT_CODE" -eq 0 ]; then
  event "workload" "succeeded"
  write_status "running" "$RUNPOD_EXIT_CODE" "workload_succeeded"
else
  event "workload" "failed"
  write_status "failed" "$RUNPOD_EXIT_CODE" "workload_failed"
fi
...
if [ "$RUNPOD_EXIT_CODE" -eq 0 ] && [ "$RUNPOD_EGRESS_EXIT_CODE" -ne 0 ]; then
  ...
  event "egress" "failed"
  write_status "failed" "$RUNPOD_EXIT_CODE" "egress_failed"
elif [ "$RUNPOD_EXIT_CODE" -eq 0 ]; then
  event "egress" "completed"
  write_status "succeeded" "$RUNPOD_EXIT_CODE" "complete"
fi
...
terminal_hold
stop_progress_server
if [ "$RUNPOD_TERMINAL_HOLD_RAN" != "1" ]; then
  inspection_hold
fi
...
HASH_PATH = Path(os.environ.get("RUNPOD_HASH_FILE", "runpod-execution/artifact_hashes.jsonl"))
...
LOG_TAIL_BYTES = max(0, min(int(os.environ.get("RUNPOD_PROGRESS_LOG_TAIL_BYTES", "4096") or 4096), MAX_LOG_TAIL_BYTES))
...
def health_payload() -> dict:
    ...
    heartbeat_phase = str(heartbeat.get("phase") or "")
    harness_alive = bool(heartbeat and heartbeat_source == "cloud_bridge")
    payload = {
        ...
        "monitor_alive": harness_alive,
        "workload_progress_unproven": heartbeat_phase == "monitor",
        "productive_hint": None,
        "productive_hint_reason": "progress endpoint reports liveness; compare repeated samples for log/status advancement before claiming workload progress",
    }
    return payload
...
class Handler(BaseHTTPRequestHandler):
    ...
    def tail_authorized(self) -> tuple[bool, int, str]:
        if not INCLUDE_LOG_TAIL:
            return False, 404, "log tail disabled"
        if not TOKEN:
            return False, 403, "log tail requires a progress token"
    ...
    def do_GET(self):
        if self.path in ("/", "/healthz", "/progress.json"):
            ...
            return
        if self.path == "/tail":
            ok, status_code, error = self.tail_authorized()
            if not ok:
                self.write_json(status_code, {"ok": False, "error": error})
                return
