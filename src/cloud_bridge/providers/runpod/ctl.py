...
def build_pod_create_command(manifest: dict[str, Any]) -> list[str]:
    ...
    combined_env = {str(key): str(value) for key, value in body_env.items()}
    combined_env.update(build_bridge_managed_env(manifest))
    ...
    if combined_env:
        add_flag(args, "--env", json.dumps(combined_env, sort_keys=True))
