...
SKILL_PATH = BRIDGE_ROOT / "skills" / "cloud-symphony" / "SKILL.md"
NORMAL_SKILL = Path.home() / ".codex" / "skills" / "cloud-symphony"
REPO_LOCAL_SKILL = BRIDGE_ROOT / ".codex" / "skills" / "cloud-symphony"
SYMPHONY_HOME = Path(os.environ.get("RUNPOD_BRIDGE_SYMPHONY_HOME", str(Path.home() / ".symphony" / "codex-home-symphony")))
SYMPHONY_SKILL = SYMPHONY_HOME / "skills" / "cloud-symphony"
SYMPHONY_CONFIG = SYMPHONY_HOME / "config.toml"
SYMPHONY_BIN = Path(os.environ.get("RUNPOD_BRIDGE_SYMPHONY_BIN", str(Path.home() / ".symphony" / "bin" / "cloud-bridge")))
DEFAULT_SYMPHONY_ENV = Path.home() / ".symphony" / "env.sh"
...
def run_doctor() -> dict[str, Any]:
    ...
    add_link_check(checks, "symphony_cli_wrapper", SYMPHONY_BIN, BRIDGE_ROOT / "bin" / "cloud-bridge", missing_status="warn")

    config_status, config_message = check_symphony_config()
    add("symphony_skill_config", config_status, config_message)

    local_wrapper = BRIDGE_ROOT / "bin" / "cloud-bridge"
    ...
    symphony_env = symphony_env_path()
    add("symphony_env", "pass" if symphony_env.is_file() else "warn", str(symphony_env) if symphony_env.is_file() else f"optional env file not found: {symphony_env}")
    add(
        "RUNPOD_API_KEY",
        "pass" if os.environ.get("RUNPOD_API_KEY") else "warn",
        "present" if os.environ.get("RUNPOD_API_KEY") else missing_env_message("RUNPOD_API_KEY", symphony_env),
    )
...
def symphony_env_path() -> Path:
    return Path(os.environ.get("RUNPOD_BRIDGE_SYMPHONY_ENV", str(DEFAULT_SYMPHONY_ENV))).expanduser()


def missing_env_message(name: str, symphony_env: Path) -> str:
    message = f"missing in current shell; {name} is required only for remote launch"
    if symphony_env.is_file():
        message += f"; source centralized env first: set -a && . {symphony_env} >/dev/null 2>&1 && set +a"
    return message
