    ...
    *,
    ...
    if live_progress_port:
        signal: dict[str, Any] = {
            ...
            "proves": "workload phase, status file presence, heartbeat freshness, and log byte count while the workload is still running",
            "does_not_prove": "artifact success or scientific/domain success",
        }
    ...
    if access.get("ssh_required") is True:
        commands.append(f"runpod-bridge pod-ssh-info {pod_id}")
    ...
    commands.append(f"runpod-bridge runtime-metrics {pod_id} --expected-elapsed-seconds ELAPSED_SECONDS --json")
    signals.append(
        {
