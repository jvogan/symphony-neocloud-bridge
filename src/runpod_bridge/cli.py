...
from .manifest import build_plan, dumps_pretty, load_manifest, validate_manifest
...
from .runpod_runtime import RunpodGraphqlError, build_runtime_metrics_report, load_previous_report
from .runpodctl import (
    RunpodCtlError,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="runpod-bridge")
    ...
    if args.command == "prepare":
        manifest = load_manifest(args.manifest)
        preflight = prepare_packet(manifest, args.out_dir)
        if args.as_json:
    ...
    if args.command == "provider-capabilities":
        ...
        if args.as_json:
            ...
            print(f"implemented: {str(capabilities.get('implemented')).lower()}")
            print(f"adapter: {capabilities.get('adapter')}")
        return 0 if capabilities.get("implemented") else 2
    ...
    if args.command == "monitor":
        manifest = load_manifest(args.manifest)
        report = inspect_execution(manifest, args.base_dir)
        if args.as_json:
            sys.stdout.write(json.dumps(report, indent=2, sort_keys=True) + "\n")
        else:
        ...
        return 0 if report["state"] in ("running", "succeeded") else 1
    ...
    if args.command == "cleanup-pod":
        if args.execute and not args.yes_cleanup_runpod:
            ...
            return 2
        try:
            record = cleanup_pod_flow(args.pod_id, out_dir=args.out_dir, action=args.action, execute=args.execute)
        except (RunpodRestError, ValueError) as exc:
            print(str(exc), file=sys.stderr)
            return 1
        if args.as_json:
        ...
        return 0 if record["status"] in ("dry_run_request", "submitted", "already_absent") else 1
