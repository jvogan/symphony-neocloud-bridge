...
from .runpod_rest import RunpodRestClient, now, redact
...
def build_provider_handoff(
    ...
    *,
    ...
    status = "ready_for_orchestrator" if plan["remote_ready"] else "blocked"
    files = {
...
def validate_provider_handoff(handoff: dict[str, Any], *, handoff_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if not manifest_path:
        ...
        if not plan["remote_ready"]:
            error("manifest.plan.remote_ready", "launch manifest is not remote-ready")
