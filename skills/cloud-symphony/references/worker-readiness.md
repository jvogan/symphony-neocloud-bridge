...
- `cloud-bridge doctor` must find the bridge CLI and the `cloud-symphony` skill.
- `cloud-bridge list-pods --name-prefix definitely-no-match --json` must succeed before a worker performs paid mutation from inside its shell.
...
```bash
cloud-bridge validate-manifest <manifest>
cloud-bridge audit-manifests <repo-or-template-dir>
cloud-bridge contract-self-check <manifest>
cloud-bridge preflight <manifest>
cloud-bridge prepare <manifest> --out-dir .runtime/<run-id>-packet
cloud-bridge validate-handoff .runtime/<run-id>-packet/provider_handoff.json
```

If the image comes from GHCR, private ECR, GitLab registry, Artifact Registry, or another private registry, the preflight must also prove provider-side image pullability. Configure RunPod registry auth on the trusted orchestrator or record an exact pull canary as `runpod.image_pull_verified: true`; local worker registry login is not enough.

For long, expensive, large, or huge runs, also require:

```bash
cloud-bridge source-check <manifest> --execute
cloud-bridge egress-plan <manifest>
cloud-bridge profiles --recommend-for <manifest>
cloud-bridge productivity-plan <manifest>
```
...
For long batch jobs, configure a terminal hold rather than relying on container exit semantics:
...
The workload writes final `status.json`, logs, hashes, and artifacts, then idles until the orchestrator fetches evidence and deletes the pod. This prevents a terminal batch command from being mistaken for a crash or restart loop.
...
3. Registry canary: proves RunPod can pull the exact private image with provider-side registry auth.
4. Volume canary: proves network volume mount and file visibility when the workload needs retained storage.
5. GPU canary: proves scheduler, image, driver visibility, and `nvidia-smi` before running real GPU code.
6. Full workload: only after artifact egress, hashing, and cleanup have already been proven on the same route.
...
Monitor loops should use `progress-report --previous ... --out ...` and report `classification.state` verbatim. `monitor_alive` means the bridge wrapper is still reachable; `workload_progressing` is true only when repeated samples show advancing workload evidence. Do not translate monitor heartbeat into green workload progress.
